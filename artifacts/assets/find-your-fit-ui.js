(function (root, factory) {
  ...
  function syncSettingInputs(inputs, changedInput) {
    const anySetting = inputs.find(function (input) { return input.value === "any"; });
    if (!anySetting) return;
    if (changedInput === anySetting && changedInput.checked) {
      inputs.filter(function (input) { return input !== anySetting; }).forEach(function (input) {
        input.checked = false;
      });
    } else if (changedInput !== anySetting && changedInput.checked) {
      anySetting.checked = false;
    }
    if (!inputs.some(function (input) { return input.checked; })) anySetting.checked = true;
  }
