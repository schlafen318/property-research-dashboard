(function (root) {
  ...
  function initRetirementCalculator(rootId, payload) {
    ...
    function syncDestinationDefaults() {
      const record = selectedRecord();
      if (!record) return;
      const profile = record.profiles[el("ret-household").value];
      const plan = el("ret-housing-plan").value;
      benchmarkValue = Object.values(profile.categories_usd).reduce(function (total, value) {
        return total + Number(value);
      }, 0) + Number(housingAmount(profile, plan));
      el("ret-spending").value = String(Math.round(benchmarkValue));
    ...
    function expenseCategories(record, profile, plan) {
      ...
      const scale = benchmarkValue > 0 ? number("ret-spending") / benchmarkValue : 1;
    ...
    function calculatorInput() {
      ...
      const input = {
        ...
        housingPlan: plan,
        propertyPrice: Number(record.property.representative_price_usd),
  ...
  root.GHARetirementCalculatorUI = { initRetirementCalculator: initRetirementCalculator };
})(window);
