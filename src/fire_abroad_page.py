        ...
        if not row["rankable"]:
            rendered.append(
                ...
                <tr>
                  ...
                  <td colspan="4"><strong>Research pending</strong> — this destination is not assigned a zero-tax assumption or ranked until its evidence is complete.</td>
                </tr>
                """
            )
            continue
...
def _property_lifecycle(countries: dict[str, Any]) -> str:
    complete = next(
        (country["tax_screen"] for country in countries.values() if country["tax_screen"].get("status") == "complete"),
        None,
    )
    if not complete:
        return ""
    lifecycle = complete["property_lifecycle"]
    stages = (
        ("Purchase", "purchase"),
...
{head}
  <style>
    ...
    .fire-refine{{min-height:44px;padding:10px 15px;border:1px solid #5f7f72;background:#fff;color:#24312d}}
...
<main>
  ...
  <section class="fire-results" aria-labelledby="fire-results-heading"><div class="fire-shell">
    ...
    <div class="fire-table-wrap"><table class="fire-table"><thead><tr><th>Destination</th><th>FIRE score</th><th>Likely tax residence</th><th>Tax Readiness</th><th>Planning tax reserve</th><th>Next step</th></tr></thead><tbody id="fire-results-body">{_result_rows(rows)}</tbody></table></div>
  </div></section>
  <section class="fire-method"><div class="fire-shell">
    <h2>Foreign-home tax lifecycle</h2>
    <p>Property tax does not stop at the purchase. Open these stages before relying on a home budget.</p>
    <dl class="fire-lifecycle">{_property_lifecycle(countries)}</dl>
    <p><button class="fire-refine" type="button" data-fire-refine>Refine this tax estimate</button></p>
    <details><summary>Assumptions and primary sources</summary><p>Data checked {escape(reviewed)}. Complete destinations use current primary-source evidence; incomplete destinations remain visibly unranked.</p><ul class="fire-sources">{_sources(sources)}</ul></details>
  </div></section>
</main>
{footer}
