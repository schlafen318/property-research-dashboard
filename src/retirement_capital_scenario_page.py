...
def _destination_row(item: dict, destination: dict) -> str:
    ...
    country_link = (
        ...
        f'data-track-label="country {escape(item["destinationId"])}">Country guide</a>'
    ...
    return f"""
      <article class="scenario-row">
        ...
        <nav><a href="/destinations/{escape(item['destinationId'])}/" data-track="retirement_capital_scenario_guide_click" data-track-label="destination {escape(item['destinationId'])}">Destination guide</a>{country_link}</nav>
      </article>
    """.strip()
...
</style></head><body class="gha-mode-utility retirement-scenario-page">{navigation}
...
<main><div class="scenario-shell"><section><p class="scenario-eyebrow">Planning estimate</p><h2>What this comparison assumes</h2><dl class="scenario-assumptions"><div><dt>Capital at retirement</dt><dd>{label}</dd></div><div><dt>Household</dt><dd>Couple</dd></div><div><dt>Retirement period</dt><dd>30 years</dd></div><div><dt>Housing</dt><dd>Rent</dd></div></dl></section>
<section><h2>{'Destinations Within reach' if within else 'Closest destinations'}</h2>{rows}</section>
<section><h2>Test your retirement plan</h2><p>Use your own timeline, capital, income and housing plan.</p><a class="scenario-action" href="/retirement-destination-finder/" data-track="retirement_capital_scenario_calculator_start" data-track-label="{escape(slug)}">Open the retirement finder</a></section>
