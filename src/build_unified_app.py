  ...
  <main><div class="calc-shell">
    <section class="calculator-layout" aria-label="Retirement calculator">
      ...
      <section class="calc-panel result-panel" id="ret-results" aria-live="polite" aria-atomic="true">
        ...
        <div class="result-total" id="ret-total-capital">—</div><p id="ret-headline-label">Retirement capital needed at retirement</p>
        <div class="result-grid"><div><span>Liquid portfolio</span><strong id="ret-liquid-portfolio">—</strong></div><div><span id="ret-property-label">Home purchase</span><strong id="ret-property-capital">—</strong></div><div><span>Emergency reserve</span><strong id="ret-emergency-reserve">—</strong></div><div><span>Retirement capital in today's dollars</span><strong id="ret-today-total">—</strong></div><div><span>First-year spending</span><strong id="ret-first-expenses">—</strong></div><div><span>Outside income</span><strong id="ret-outside-income">—</strong></div><div><span>First-year funding gap</span><strong id="ret-funding-gap">—</strong></div><div><span>Expected return after fees</span><strong id="ret-result-return">—</strong></div><div><span>Implied first-year withdrawal</span><strong id="ret-result-implied-withdrawal">—</strong></div></div>
    ...
    <section id="methodology" class="content-section"><h2>How the calculation works</h2><p>The personalized calculator projects each annual expense through retirement using its inflation assumption. It separately projects inflation-linked reliable income, keeps fixed income nominal, and floors each year's funding gap at zero.</p><p>Each annual funding gap is discounted to the retirement date using the expected portfolio return you enter. Their sum is the liquid portfolio required to fund the modeled horizon, assuming a constant return every year and a zero ending liquid balance. The first-year gap divided by that portfolio is shown as an implied withdrawal, not a recommended or safe withdrawal rate.</p><p><strong>Portfolio dividends and interest</strong> belong inside the expected total return and are not subtracted again as outside income. The model does not simulate volatility, sequence-of-returns risk, investment taxes, fees beyond your after-fee return assumption, or a bequest. Actual results can differ materially.</p><p>Emergency reserves remain separate from the liquid portfolio. A home bought now is shown in today's USD and is never mixed with retirement-year capital. A home bought at retirement is projected to that date and can be combined with retirement capital. Exchange rates establish a current comparison baseline and are not forecasts.</p><h3>Sources and confidence</h3><p>Data reviewed __AS_OF__. Destination observations and documented country-level proxies are planning inputs, not precise quotes.</p><ul>__SOURCES__</ul></section>
