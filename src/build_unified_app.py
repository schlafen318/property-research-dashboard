    ...
    <section class="calculator-layout" aria-label="Retirement calculator">
      <form class="calc-panel" id="retirement-calculator" novalidate>
        ...
        <fieldset><legend>Destination and housing</legend><div class="field-grid">
          <div class="field"><label for="ret-destination">Destination</label><select id="ret-destination">__OPTIONS__</select></div>
          <div class="field"><label for="ret-housing-plan">Housing plan</label><select id="ret-housing-plan"><option value="rent">Rent</option><option value="own">Already own</option><option value="buy_now">Buy now</option><option value="buy_retirement" selected>Buy at retirement</option></select></div>
          <div class="field"><label for="ret-monthly-spending">Monthly retirement living expenses (today's USD)</label><input id="ret-monthly-spending" type="number" min="0" step="1" value="0"><p class="hint" id="ret-housing-guidance">Monthly retirement living expenses after purchase, including owner running costs but not the home purchase at retirement.</p></div>
          <div class="field" id="ret-property-field"><label for="ret-property-budget">Home purchase budget today (USD)</label><input id="ret-property-budget" type="number" min="0" step="1" value="0"><p class="hint">Prefilled with today's representative destination price. Edit it to match the home you expect to buy; acquisition costs are added separately.</p></div>
        </div></fieldset>
        <fieldset><legend>Reliable after-tax income</legend><p class="hint">Use net amounts expected to continue in retirement. Do not include dividends from the portfolio being calculated.</p><div class="field-grid">
          <div class="field"><label for="ret-pension">Annual pension (USD)</label><input id="ret-pension" type="number" min="0" step="100" value="24000"><label class="check"><input id="ret-pension-indexed" type="checkbox" checked> Inflation-linked</label></div>
          <div class="field"><label for="ret-other-income">Other non-portfolio income (USD)</label><input id="ret-other-income" type="number" min="0" step="100" value="18000"><label class="check"><input id="ret-other-indexed" type="checkbox"> Inflation-linked</label></div>
          <div class="field"><label for="ret-rental-income">Other net rental income (annual USD)</label><input id="ret-rental-income" type="number" min="0" step="100" value="0"><p class="hint">Only include income from a separate rental property. Leave at $0 when your destination home is for your own use.</p><label class="check"><input id="ret-rental-indexed" type="checkbox"> Inflation-linked</label></div>
        </div></fieldset>
        ...
        <div id="ret-errors" role="alert" tabindex="-1"></div><button class="primary" id="ret-calculate" type="submit">Calculate retirement capital</button>
      </form>
      <section class="calc-panel result-panel" id="ret-results" aria-live="polite" aria-atomic="true">
        ...
        <section class="result-period" id="ret-today-section" aria-labelledby="ret-today-heading"><h3 id="ret-today-heading">What you need today</h3>
          <div class="result-total" id="ret-total-today">—</div><p>Total to allocate today</p>
          <div class="result-grid"><div><span>Invest today for retirement</span><strong id="ret-invest-today">—</strong></div><div><span id="ret-home-today-label">Home purchase needed now</span><strong id="ret-home-today">—</strong></div></div>
          <p class="hint">The investment amount assumes it earns your expected return every year until retirement.</p>
        </section>
...
def build_retirement_destinations_article(destinations: list[dict], retirement_payload: dict) -> str:
    ...
    ranking_header = """<thead><tr>
      ...
      <th scope="col" aria-sort="none"><button type="button" class="sort-button" data-sort-key="annual"><span>Annual cost</span><span class="sort-indicator" aria-hidden="true">↕</span></button></th>
    ...
    for rank, item in enumerate(rankings, start=1):
        ...
        table_rows.append(
            f"""
            <tr class="ranking-row" data-rank="{rank}" data-name="{escape(destination["name"])}" data-annual="{metrics["annual_spending"]}" data-savings="{metrics["required_capital"]}" data-property="{metrics["property_capital"]}">
              <td>{rank}</td>
...
def build_seo_page(
    ...
    if page["slug"] == "japan-retirement-property-foreign-buyers":
        editorial_content = """
          ...
          <section class="seo-section"><h2>The practical purchase sequence</h2><p>Start with an extended stay, not a listing feed. Choose two locations, rent in each, and view homes with an independent interpreter or bilingual professional. Once a property survives that test, confirm ownership, hazard, management, and running-cost questions before negotiating price.</p><p>Keep your advisers separate: an agent may help identify property, but a legal professional, tax adviser, and building inspector should be able to give an independent view. A clean purchase is one where you can explain the asset, the documents, the costs, and the exit route without relying on a sales narrative.</p></section>
          <section class="seo-section"><h2>The cost of ordinary life</h2><p>Japan is not one cost-of-living market. Tokyo is the expensive reference point; Fukuoka offers the useful alternative of a full city—good hospitals, an airport, serious restaurants and day-to-day convenience—without asking you to organise your life around Tokyo. The Statistics Bureau publishes regional price comparisons for a reason: housing, food, transport and services shift materially by city.</p><p>For an owner, the important distinction is between a pleasant visiting budget and a durable annual one. In Fukuoka, the recurring question is urban convenience versus a coastal address. In Hakone, Izu, Hakuba and Niseko, it is seasonal: heating, snow clearance, car use, resort premiums, management and the cost of keeping a house ready when you are away. Put those expenses in a separate annual operating budget, alongside property tax, insurance, repairs and furnishing; do not let a low purchase price disguise a high-friction second home.</p><p>Finance is its own workstream. Confirm what a lender will offer to your residency and income profile before leverage shapes the shortlist. Non-resident reporting is not a footnote: Japan's Ministry of Finance says a report under FEFTA is generally required for a non-resident acquiring Japanese real property or rights in it, and must be submitted via the Bank of Japan within 20 days after acquisition. Confirm whether an exemption applies with Japanese counsel before completion.</p></section>
          <section class="seo-section"><h2>Healthcare: excellent once you are in the system</h2><p>A property is not a route into Japanese healthcare. Public-health-insurance eligibility follows residence and the relevant visa or employment status; property ownership alone does not create eligibility. The Ministry of Health notes that foreign residents who meet the requirements, including qualifying residence in Japan, can use the public system and receive care at medical institutions with a defined patient co-payment.</p><p>For a buyer planning a real move, test the practical version of that promise: find a clinic, dentist, pharmacy and hospital near the home, then ask how appointments, records and consent work in English. Large-city access is easier. Resort and rural areas can feel wonderfully calm until you need a specialist, an interpreter, or urgent care outside a weekday routine. Keep private international cover in place until your residency, insurance and local registration position are fully settled.</p></section>
          <section class="seo-section"><h2>The lifestyle is built from small rituals</h2><p>Japan rewards a buyer who enjoys the texture of ordinary life: a reliable local station, a neighbourhood sento or onsen, a good depachika or market, an evening walk that feels safe, and the seasonal calendar changing what is on the table. It is less forgiving if you want everything arranged in English, on demand, without adapting to local rhythms.</p><p>Fukuoka is the easiest of the four to make routine-rich: food, city energy and Kyushu weekends are close together. Hakone and Izu are more contemplative—beautiful when you value hot springs, gardens and a quieter week, but more dependent on the exact road, slope and season. Hakuba and Niseko give a committed winter life rather than a generic mountain escape. Visit in the inconvenient month as well as the beautiful one; the version of the place you can use in rain, humidity, snow or shoulder season is the one you are buying.</p></section>
          <section class="seo-section"><h2>Travel is part of the location decision</h2><p>Test the journey door to door, not city to city. Fukuoka is unusually persuasive because the airport is embedded in the city: Japan National Tourism Organization notes that Hakata is a five-minute train ride from Fukuoka Airport. That matters when a home is used often, when family visit, or when a repair requires a quick trip rather than a carefully planned holiday.</p><p>Hakone and Izu work best when Tokyo access and a comfortable last mile are both real, not assumed from a map. Hakuba and Niseko require a more deliberate winter-transfer plan: flight timing, road conditions, luggage, a car or driver, and who opens the house before arrival. For every shortlist, make one timed journey from your usual international gateway, with groceries and a delayed flight in the scenario. A beautiful home that takes too much effort to reach becomes a home you use less.</p></section>
          <section class="seo-section"><h2>The 48-hour neighbourhood test</h2><p>Spend two ordinary days where you expect to own. Take the first train, buy groceries, find a clinic, walk home after dark, and try to resolve one practical task without a hotel concierge. In a resort market, repeat the exercise outside the peak season. The property should make sense when the scenery is not doing all the work.</p></section>
          <section class="seo-section"><h2>Primary sources to use with your advisers</h2><p><a href="https://www.mof.go.jp/english/policy/international_policy/real_property/index.html">Japan's Ministry of Finance: reporting requirements for non-residents acquiring real property</a>; <a href="https://www.mlit.go.jp/report/press/totikensangyo13_hh_000269.html">MLIT: English introduction to Japan's real-estate transaction, registration, tax, and planning systems</a>; <a href="https://www.mhlw.go.jp/content/12400000/001406614.pdf">Ministry of Health, Labour and Welfare: health insurance for foreign residents</a>; <a href="https://www.stat.go.jp/english/data/nenkan/74nenkan/1431-20.html">Statistics Bureau: regional consumer-price comparisons</a>; <a href="https://faq.japan-travel.jnto.go.jp/en/plan/airport-access/fukuoka-airport/">JNTO: Fukuoka Airport access</a>; <a href="https://www.cao.go.jp/tochi-chosa/index_en.html">Cabinet Office: Act on Review of Important Real Estate</a>.</p></section>
        """
    generic_intro = f"""
