...
- The Hong Kong domestic-residence day test now evaluates both pairs of consecutive assessment years containing the relevant year: current plus previous and current plus following. It applies the agreement's strict “more than 300 days” threshold, validates each day count as an integer from 0 through 366, and treats an unknown future/following year as unresolved whenever it could change the result.
- The ordinary-residence legal self-classification was removed. The detailed screen instead asks whether Hong Kong remained the place of normal settled daily life and collects the objective fixed-home, Hong Kong work/business, and close-family facts listed in [IRD personal-assessment guidance](https://www.ird.gov.hk/eng/faq/pa.htm). `Not sure` and mixed factual signals fail closed; the copy explicitly says the user is supplying facts rather than a legal conclusion.
- When every property overlay has `taxpayerScope: not_applicable`, the audit now omits the numeric one-time property-tax total and its purchase/sale/inheritance/gift additive formula. The property audit and reconciled table retain only the explicit renter `N/A` treatment.
