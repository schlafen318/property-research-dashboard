...
class GuideHubIndexingTests(unittest.TestCase):
    ...
    def test_japan_guide_has_country_specific_buyer_content_and_primary_sources(self) -> None:
        ...
        self.assertIn("The cost of ordinary life", text)
        self.assertIn("Healthcare: excellent once you are in the system", text)
        self.assertIn("The lifestyle is built from small rituals", text)
        self.assertIn("Travel is part of the location decision", text)
        self.assertIn("Hakata is a five-minute train ride from Fukuoka Airport", text)
        self.assertIn("property ownership alone does not create eligibility", text)
