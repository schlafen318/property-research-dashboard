...
class GuideHubIndexingTests(unittest.TestCase):
    ...
    def test_japan_guide_has_country_specific_buyer_content_and_primary_sources(self) -> None:
        ...
        self.assertIn("Live well, year after year", text)
        self.assertIn("Reach it easily—and feel at home there", text)
        self.assertIn("Own and operate cleanly", text)
        self.assertIn("Income and upside need different stories", text)
        self.assertIn("Preserve the exit—and the entry discipline", text)
        self.assertNotIn("Lifestyle magnetism 10%", text)
        ...
        self.assertIn("A long-stay residence status comes first", text)
        self.assertIn("register an address with the municipality", text)
        self.assertIn("more than three months", text)
        self.assertIn("flood-hazard map", text)
        self.assertIn("Real estate acquisition tax and registration licence tax", text)
        self.assertIn("Before signing, the agent's Important Matters Explanation", text)
